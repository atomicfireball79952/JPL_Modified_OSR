...
class Robot():
	...
	'''
	def __init__(self):
		distances = rospy.get_param('mech_dist','5.75,11.75,11.75,9.125').split(",")
	...
	def calculateTargetDeg(self,radius):
		...
		ang4 = int(math.degrees(math.atan(self.d3/(abs(r)+self.d1))))
		ang3 = int(math.degrees(math.atan(self.d2/(abs(r)+self.d1))))
		ang2 = int(math.degrees(math.atan(self.d2/(abs(r)-self.d1))))
